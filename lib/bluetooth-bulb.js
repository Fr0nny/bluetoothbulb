...
(function() {
	...
	const BULBS = [
		...
		/* Playbulb */
		{
			'filters':	[ { namePrefix: 'Fulife' } ],
			'optionalServices':	[ 0x2901 ],

			'write':	{
				'service':			0x180A,
				'characteristic':	0x2901,
			...
			'read': {
				...
				'interpret':		function(buffer) {
					return { r: buffer.getUint8(1), g: buffer.getUint8(2), b: buffer.getUint8(3) }
				}
			}
		},
            ...
            return new Promise((resolve, reject) => {
	            navigator.bluetooth.requestDevice({
		            ...
		            .then(device => {
		                ...
		                this._BULB = BULBS.filter(item => {
							return item.filters.filter(filter => filter.namePrefix && device.name.indexOf(filter.namePrefix) === 0).length;
		                }).pop();
